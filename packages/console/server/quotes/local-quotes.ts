 ...
 * error.
 */
export async function loadLocalQuotes(dir: string): Promise<Map<string, DatumQuote>> {
  const files = await readdir(dir).catch(() => [] as string[]);
  const map = new Map<string, DatumQuote>();
  await Promise.all(
    files
      .filter((f) => f.endsWith(".json"))
      .map(async (f) => {
        const quote = JSON.parse(await readFile(join(dir, f), "utf-8")) as DatumQuote;
