...
A settlement produces two kinds of record: an on-chain transaction (`DatumEscrow.settle`,
...
| Field               | Type                       | Meaning                                                                                                                                                                                                         |
| ------------------- | -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `schema_version`    | string, `"1.0"` in build 1 | Same forward-compatibility convention as `datum-quote`.                                                                                                                                                         |
| `quote_hash`        | bytes32 hex                | Binds this receipt to the specific `datum-quote` it settles — matches `DatumEscrow.openAndFund`'s `quoteHash` param (build1-spec.md §10) and `quote/sign.ts`'s `quoteHashHex`.                                  |
...
| `amount_paid_usd`   | decimal string             | **Gross**: the `actualAmount` passed to `DatumEscrow.settle`, _before_ the `feeBps` treasury split. See "Gross vs. net" below — build1-spec.md §10 doesn't state this either way, and this document settles it. |
...
| `signature`         | hex string                 | Datum's signature over every other field, proving the receipt itself wasn't altered after being issued.                                                                                                         |
...
to what a payment did, signed by Datum, not by the seller. If a future version needs the seller
to co-sign settlement, that is an additive field, not a reason to alter what exists.

## Gross vs. net

`amount_paid_usd` is defined here as **gross**: the full `actualAmount` `DatumEscrow.settle`
...
a gross ceiling — `DatumEscrow`'s own invariant is `actualAmount ≤ maxAmount`, both gross figures.
...
`DatumEscrow`'s own invariant, `actualAmount ≤ maxAmount`. `receipt/verify.ts`'s `verifyReceipt`
...
`quote_hash` ties a receipt to the exact `datum-quote` it settles, using the same `quoteHash`
...
**independently verifiable offline** — a third party with the receipt and Datum's public key
