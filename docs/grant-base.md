...
| 4   | MCP server live, listed, and has taken at least one real paid call from an agent that is not Datum's own | **Not met.** `packages/mcp-server`'s four tools and Circle Gateway paywall integration are implemented and tested, but the server is not deployed to a public, always-on endpoint, and is not listed on any MCP registry or the Circle Agent Marketplace. Every call made against it so far has been from this project's own tests and demo agents. |
| 5   | Escrow contract deployed to testnet with green tests and no admin path to funds                          | **Met.** `DatumEscrow` and `DatumAttestation` are deployed and verified on Base Sepolia (chain 84532) — see `data/deployments/base-sepolia.json`. 50+ Foundry unit tests, a reentrancy suite, and four fuzzed invariant tests (256 runs × 16,384 calls each) all pass. Zero privileged roles exist in the contract by construction.                 |
| 6   | `datum-quote` published as a draft spec                                                                  | **Met.** `docs/datum-quote.md`.                                                                                                                                                                                                                                                                                                                     |
| 7   | Demo agents complete a full quote → compare → escrow → settle → verify loop on testnet                   | **Met.** `packages/agents`, run live against Base Sepolia — real HTTP 402 responses, real on-chain escrow funding and settlement, a real signed receipt. Explicitly labelled a testbed: these are Datum's own agents on Datum's own testnet wallets, demonstrating the protocol, not market demand.                                                 |
...
Datum publishes Dated SIU — the benchmark price of AI inference work. One SIU (Standard Inference
...
Datum is a measurement standard and data publication. It is not a currency, not a stablecoin, and
...
- **On-chain contracts, deployed and tested on Base Sepolia** — `DatumAttestation` (immutable
  publisher-anchored hash timestamping) and `DatumEscrow` (non-custodial: funds reach only the
...
- **`datum-quote`** (`docs/datum-quote.md`) — an x402/MPP payment-required response extension
...
- **Independent, key-distribution-free verification** — recover a print's signer from its raw
  signature, compare against `DatumAttestation.publisher()` on chain, confirm the body hash is
...
new token. `DatumEscrow`'s constructor takes the settlement token and treasury as parameters and
...
1. **`treasury` must be a Safe/multisig, not an EOA.** It is immutable with no setter in
   `DatumEscrow`, so a lost treasury key would make fee revenue permanently unrecoverable — the
   only remedy would be redeploying the escrow and migrating every integrator.
2. **`DATUM_PUBLISHER_KEY` must be generated on an air-gapped machine and held in a secrets
...
them: security audit of `DatumEscrow` prior to mainnet deployment, ongoing infrastructure hosting
...
- **Single-operator governance risk.** Datum currently controls its own methodology and harness.
