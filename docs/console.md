...
_`packages/console` — a localhost analytics board for operating Datum: reading every print,
...
- **Read-only.** This package never imports a write-capable function from any `@datum/*`
...
- **`.env` is read, never written, and no key material is ever read.** The console reads
  `BASE_SEPOLIA_RPC_URL`/`DATUM_CHAIN_NAME`/`DATUM_PUBLISHER_ADDRESS` and displays addresses as-is.
  `DATUM_PUBLISHER_KEY` and `DEPLOYER_PRIVATE_KEY` are never touched by any file in this package.
...
- **Reuse, not reimplementation.** Every panel's data comes from an existing exported function in
  `@datum/sdk`/`@datum/print` — `loadPrint`, `verifyPrint`, `recoverSignerCandidates`,
...
Needs `.env` sourced first (`BASE_SEPOLIA_RPC_URL`, `DATUM_CHAIN_NAME`, `DATUM_PUBLISHER_ADDRESS` —
...
(`recoverSignerCandidates` → compare against `DatumAttestation.publisher()` → confirm the body
...
**4. Agent activity.** Every escrow lifecycle reconstructed from `DatumEscrow`'s own
...
(`@datum/sdk`'s `minorUnitsToUsd`/`D`), **not** by calling `verifyReceiptTool` itself, which needs
`DATUM_PUBLISHER_KEY` to sign a receipt and this read-only package must never hold or use a
...
`DatumEscrow`'s `Settled` event carries only `quoteHash` — the full quote (SIU amount, model,
which print it referenced) never appears on chain, only its hash. Confirmed by direct search: no
file anywhere in this repo persisted a `datum-quote` after building it until this console's build
