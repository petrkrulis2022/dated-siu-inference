...
9. **The per-print seed (§3) is generated before the print ships but only published after — where does it live in the interim?** If it sits in a plaintext working-tree file pre-publication, anyone with repo/CI access sees it early, which undercuts the "nobody can pre-train against next week's tasks" claim. *Resolution:* keep the pre-publication seed out of version control (generate at run time from a local secret, write it into `data/runs/` only once the print that used it has shipped).

---

## 4. Where the spec and CLAUDE.md's invariants are in tension

1. **CLAUDE.md's Build 2 design consequence — "the receipt and quote formats must be written so a token wrapper is additive later, never a rewrite" — isn't yet satisfied by §8's concrete `datum-quote`/receipt field lists.** Neither schema reserves an extension point (e.g. a `settlement_asset` field, defaulted to `"usdc"`) for a future wSIU-denominated payment. This is the clearest actual gap between an invariant and the spec's literal fields — worth closing in the M1 schema work rather than accepting a breaking change in Build 2.

2. **`DatumEscrow.settle`'s buyer-authorised "settler" role (§10) needs precise scoping against the non-custodial invariant.** As written it's structurally fine — the settler triggers a transfer but funds still only reach seller, buyer, or treasury — but it's the one place in the contract where "no admin path to user money" could erode by accident if the settler role is ever widened. Recommend an explicit test: a malicious/compromised settler cannot redirect funds anywhere outside the pre-agreed seller.
...
*Stopping here per P1. Waiting for your response before doing anything else.*
